...
void removeTail(List*& L)
{
    ...
    //duyet den phan tu truoc nut cuoi
    NODE* temp = L->p_head;
    while (temp!=NULL&&temp->p_next->p_next != NULL)
        temp = temp->p_next;
...
bool removeElement(List*& L, int key)
{
    ...
    while (temp->p_next != NULL && temp->p_next->key != key) {
        temp = temp->p_next;
    }
    ...
    NODE* deleteNode = temp->p_next;
    temp->p_next = deleteNode->p_next;
    delete deleteNode;
    return true;
}
