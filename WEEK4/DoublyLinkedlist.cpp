...
bool addHead(List*& L, int data)
{
    ...
    if (L == NULL) L = createList(p);
    else {
        p->p_next = L->p_head;
        if (L->p_head != NULL)L->p_head->p_prev = p;
        L->p_head = p;
        if (L->p_tail == NULL)L->p_tail = p;
    }
    return true;
}
...
void removeBefore(List*& L, int val)
{
    if (L == NULL || L->p_head == NULL || L->p_head->p_next == NULL)
        return;
    ...
    if (curr == NULL || curr->p_prev == NULL)
        return;

    NODE* toDelete = curr->p_prev;
    if (toDelete->p_prev != NULL)
...
bool addPos(List*& L, int data, int pos)
{
    ...
    if (pos == cnt) return addTail(L, data);
    ...
    while (curr != NULL) {
        ...
        while (runner != NULL) {
            if (runner->key == curr->key) {
                ...
                if (toDelete->p_prev != NULL)
                    toDelete->p_prev->p_next = toDelete->p_next;
                if (toDelete->p_next != NULL)
                    toDelete->p_next->p_prev = toDelete->p_prev;
                if (toDelete == L->p_tail)
                    L->p_tail = toDelete->p_prev;
...
int main()
{
    ...
    /* freeList(reversedList16);
     freeList(reversedList16_2);   dao nguoc chu khong phai tao danh sach moi, nen list va reverse list la 1 chi xoa 1 lan
